...
def train_fast(n: Network,
    ...
    Returns the final iteration reached.
...
def evaluate_performance(data: ndarray,
    ...
    for _ in range(nb_tests):
        ...
        k = train_fast(nw, data, model, it, lrng_rate, regularization, end_train)
...
def train(n: Network,
    ...
    h, it, rp, end_train = config.lrng_rate, config.it, 2 * config.r, config.end_train
    m = model.shape[0]
    layers, p = n.layers, len(n.layers) - 1
    ...
    for k in iteration_range:
        ...
        if k % 10 == 0:
            ...
            if x_t is not None and y_t is not None:
                a_test = n.forward_propagation(x_t)
                t.append(log_loss_numba(a_test, y_t))
    ...
    training_history_dict['test_loss'] = t
    training_history_dict['iterations'] = it_list
    if training_history_dict['final_iteration'] is None:
        training_history_dict['final_iteration'] = it
